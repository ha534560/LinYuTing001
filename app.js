(() => {
    ...
    applyAutoUi({
        seed: params.get('uiSeed') || cfg.uiSeed || profileId,
        ui: params.get('ui') || cfg.ui,
        variant: params.get('uiVariant') || cfg.uiVariant,
        layout: params.get('uiLayout') || params.get('layout') || cfg.uiLayout,
    });
    ...
    function renderProfile(data, id, sourceUrl) {
        ...
        applyAutoUi({
            seed: params.get('uiSeed') || cfg.uiSeed || profileId,
            ui: profile.ui || common.ui || params.get('ui') || cfg.ui,
            variant: profile.uiVariant || common.uiVariant || params.get('uiVariant') || cfg.uiVariant,
            layout: profile.uiLayout || common.uiLayout || params.get('uiLayout') || params.get('layout') || cfg.uiLayout,
        });
    ...
    function applyAutoUi({ seed, ui, variant, layout }) {
        const styles = ['magazine'];
        const layouts = ['cover', 'toc', 'stack'];
        const variants = ['a', 'b', 'c'];

        const styleOverride = normalizeUi(styles, ui);
        const variantOverride = normalizeUi(variants, variant);
        const layoutOverride = normalizeUi(layouts, layout);

        const hash = hashString(String(seed ?? 'default'));
        const style = styleOverride || 'magazine';
        const pickedVariant = variantOverride || variants[(hash >>> 6) % variants.length];
        const pickedLayout = layoutOverride || layouts[(hash >>> 12) % layouts.length];

        document.documentElement.dataset.ui = style;
        document.documentElement.dataset.variant = pickedVariant;
        document.documentElement.dataset.layout = pickedLayout;
    }

    function normalizeUi(allowed, value) {
        if (!value) return '';
